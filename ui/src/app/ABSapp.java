...
public class ABSapp {
    ...
    private void loansPlacement() {
        try {
            ...
            String customerID = InputOutput.chooseCustomerForInvestLoan(engine.getCustomerAndBalanceList());
            if (customerID != null) {
                InputOutput.printMsg("Please Insert Amount Of Money To Invest(insert positive number that will not cross your balance): ");
                int amount = InputOutput.getIntegerInput();
                int sum = Integer.parseInt(engine.getCustomerAndBalanceList().get(customerID));
                if(amount<=0)
                {
                    throw new IllegalArgumentException("You Can Invest Negative Or 0, Please Try Again");
                }
                if (sum < amount) {
